...
const App = () => {
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-gray-50 font-sans flex flex-col justify-between">
      <div>
        <header className="bg-blue-700 text-white p-6 shadow-lg text-center">
          ...
          <p className="mt-2 text-blue-100">دليلك الأول للعثور على منزل أحلامك</p>
        </header>
          ...
          {properties.map(item => (
            <div key={item.id} className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-xl transition-shadow">
              ...
              <div className="p-5">
                ...
                <div className="flex items-center text-gray-500 mt-1"><Home size={16} className="ml-1"/> {item.area}</div>
      ...
      {/* إشارة التأكد من التحديث */}
      <footer className="text-center p-8 text-gray-400 text-xs border-t bg-white">
        نظام عقارات رام الله - النسخة الحالية: <span className="font-bold text-blue-600">V 1.0</span>
      </footer>
    </div>
  );
};
