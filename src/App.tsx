...
export default function App() {
  ...
  // حالات الربط الجديدة (إدارة البيانات)
        ...
        {showLogin && !isLoggedIn && (
          <div style={{ ...s.loginBox, backgroundColor: theme.cardBg, border: `1.5px solid ${theme.border}`, boxShadow: theme.shadow }}>
            <div style={s.loginHeader}>
              ...
              <span style={{ fontWeight: 700 }}>دخول الإدارة</span>
            </div>
            <input
              type="password"
          ...
          <div style={{ ...s.admin, border: `2px solid ${theme.border}`, backgroundColor: theme.cardBg, boxShadow: theme.shadow }}>
            {adminView === 'main' ? (
              ...
              <div>
                ...
                <button onClick={handleSaveProperty} style={s.loginBtn} disabled={loading}>{loading ? 'جاري الحفظ...' : 'حفظ البيانات'}</button>
              </div>
            ) : (
              <div>
...
const s = {
  ...
  serviceCard: { display: 'flex', alignItems: 'center', gap: '20px', padding: '22px 24px', borderRadius: '22px', cursor: 'pointer', transition: '0.25s ease', outline: 'none', textAlign: 'right' },
