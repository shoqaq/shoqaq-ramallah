...
export default function App() {
  // الحالات الأصلية للتصميم
  const [isLoggedIn, setIsLoggedIn] = useState(false);
  const [showLogin, setShowLogin] = useState(false);
  const [password, setPassword] = useState('');
  const [isDarkMode, setIsDarkMode] = useState(true);

  // حالات الربط والبيانات (المحرك الخلفي)
  ...
  const handleSaveProperty = async () => {
    if (!newProperty.property_name) return;
  ...
  return (
    <div style={{ ...s.container, backgroundColor: theme.bg, color: theme.text }}>
      <div style={s.themeToggleWrap}>
        <button
          onClick={toggleTheme}
          style={{ ...s.themeBtn, border: `1.5px solid ${theme.border}`, color: theme.text, backgroundColor: theme.iconBox }}
        >
      ...
      <div style={s.mainWrapper}>
        <div style={s.identity}>
          <div
            ...
            style={{ ...s.logoWrap, border: `2px solid ${theme.border}`, boxShadow: theme.shadow }}
          >
        ...
        {!isLoggedIn && !showLogin && (
          <>
            <div style={s.grid}>
              {[
                { href: whatsappLink, icon: <svg width="24" height="24" fill="#25D366" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347"/></svg> },
                { href: 'tel:+970594560056', icon: <Phone size={24} color="#34A853" strokeWidth={1.5} /> },
                { href: 'https://instagram.com/shoqaq.ramallah/', icon: <Instagram size={24} color="#e1306c" strokeWidth={1.5} /> },
              ].map((item, index) => (
                <a
                  key={index}
                  href={item.href}
                  target="_blank"
                  rel="noreferrer"
                  style={{ ...s.box, backgroundColor: theme.iconBox, border: `1.5px solid ${theme.border}`, boxShadow: theme.shadow }}
          ...
          <div style={{ ...s.admin, border: `2px solid ${theme.border}`, backgroundColor: theme.cardBg, boxShadow: theme.shadow }}>
            {adminView === 'main' ? (
              ...
              <div>
                ...
                <input 
                  style={{ ...s.input, backgroundColor: theme.iconBox, color: theme.text }} 
                ...
                >
                  <option>شقة</option><option>أرض</option><option>فيلا</option><option>مكتب</option>
                </select>
                <button onClick={handleSaveProperty} style={s.loginBtn}><Save size={18} /> حفظ البيانات</button>
              </div>
            ) : (
              <div>
                <div style={s.adminHeader}><h3>قائمة العقارات</h3> <X size={20} onClick={() => setAdminView('main')} style={{ cursor: 'pointer' }} /></div>
...
const s = {
  ...
  themeBtn: { cursor: 'pointer', borderRadius: '14px', width: '45px', height: '45px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'transparent', transition: '0.2s ease' },
  ...
  loginBtn: { width: '100%', backgroundColor: '#f59e0b', color: '#fff', border: 'none', borderRadius: '14px', padding: '12px', cursor: 'pointer', fontWeight: '700', transition: '0.2s ease', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' },
