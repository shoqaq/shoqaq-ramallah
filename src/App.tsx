...
export default function App() {
  ...
  const theme = {
    ...
    shadow: isDarkMode ? 'none' : '0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 4px 6px -2px rgba(0, 0, 0, 0.05)', 
  };
  ...
  return (
    <div style={{...s.container, backgroundColor: theme.bg, color: theme.text}}>
      ...
      <div style={s.mainWrapper}>
        {/* الهوية البصرية - عند الضغط على اللوجو يظهر حقل كلمة المرور */}
        ...
        {isLoggedIn ? (
          <div style={{...s.admin, border: `2px solid ${theme.border}`, backgroundColor: theme.cardBg}}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '25px' }}>
              ...
              <button onClick={() => setIsLoggedIn(false)} style={s.logoutBtn}><LogOut size={22} /></button>
            </div>
            <button style={{...s.adminAction, color: theme.text, borderColor: theme.border}}><PlusCircle size={20} /> إضافة عقار جديد</button>
            <button style={{...s.adminAction, color: theme.text, borderColor: theme.border}}><LayoutDashboard size={20} /> إدارة القائمة</button>
          </div>
        ) : (
          <div style={s.services}>
            <button style={{...s.serviceCard, backgroundColor: theme.cardBg, border: `2px solid ${theme.border}`, boxShadow: theme.shadow}}>
              <Building2 size={32} color="#f59e0b" strokeWidth={1.8} />
              <span style={{...s.serviceText, color: theme.text}}>عرض الشقق المتوفرة</span>
            </button>
            <button style={{...s.serviceCard, backgroundColor: theme.cardBg, border: `2px solid ${theme.border}`, boxShadow: theme.shadow}}>
              <ClipboardEdit size={32} color="#f59e0b" strokeWidth={1.8} />
              <span style={{...s.serviceText, color: theme.text}}>تقديم طلب</span>
            </button>
            <button style={{...s.serviceCard, backgroundColor: theme.cardBg, border: `2px solid ${theme.border}`, boxShadow: theme.shadow}}>
              <Plus size={32} color="#f59e0b" strokeWidth={1.8} />
...
const s = {
  ...
  themeBtn: { border: 'none', cursor: 'pointer', borderRadius: '14px', width: '45px', height: '45px', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: '0.2s' },
  identity: { textAlign: 'center', marginBottom: '10px', display: 'flex', flexDirection: 'column', alignItems: 'center' },
  logoWrap: { cursor: 'pointer', marginBottom: '15px', borderRadius: '24px', overflow: 'hidden', width: '100px', height: '100px', transition: '0.2s' },
  logoImg: { width: '100%', height: '100%', objectFit: 'cover' },
  title: { fontSize: '1.8rem', fontWeight: '800', margin: '0', letterSpacing: '-0.5px' },
  sub: { marginTop: '5px', fontSize: '1.1rem', fontWeight: '600' },
  grid: { display: 'flex', gap: '14px', marginBottom: '45px' }, 
  box: { width: '50px', height: '50px', borderRadius: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: '0.2s' },
  services: { display: 'flex', flexDirection: 'column', gap: '16px', width: '100%', maxWidth: '380px' },
  serviceCard: { display: 'flex', alignItems: 'center', gap: '20px', padding: '24px 25px', borderRadius: '18px', cursor: 'pointer', textAlign: 'right', outline: 'none', transition: '0.2s' },
  serviceText: { fontSize: '1.15rem', fontWeight: '700' },
  input: { backgroundColor: 'transparent', border: '1.5px solid', borderRadius: '12px', padding: '12px', textAlign: 'center', width: '180px', outline: 'none', fontSize: '1rem' },
  admin: { padding: '25px', borderRadius: '24px', width: '320px' },
  adminAction: { width: '100%', padding: '12px', marginBottom: '10px', borderRadius: '12px', backgroundColor: 'transparent', border: '1px solid', textAlign: 'right', display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer', fontSize: '1rem' },
  logoutBtn: { background: 'none', border: 'none', color: '#999', cursor: 'pointer' },
  footer: { position: 'fixed', bottom: '20px', opacity: 0.6, fontSize: '10px', fontWeight: '700', letterSpacing: '1px' }
};
