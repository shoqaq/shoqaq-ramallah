...
import { createClient } from '@supabase/supabase-js'; // مكتبة الربط الرسمية
import {
  Phone, Instagram, LogOut, PlusCircle, LayoutDashboard,
  Building2, ClipboardEdit, Plus, Sun, Moon, Lock, CheckCircle2
} from 'lucide-react';

// --- إعدادات Supabase الخاصة بك (الخطوة الأولى: الربط) ---
...
export default function App() {
  ...
  const [dbStatus, setDbStatus] = useState('connecting'); // حالة الاتصال
  ...
  // دالة التأكد من الاتصال بقاعدة البيانات
  useEffect(() => {
    async function checkConnection() {
      try {
        const { data, error } = await supabase.from('_test').select('*').limit(1);
        // حتى لو الجدول غير موجود، استجابة السيرفر تعني أن الربط ناجح
    ...
    <div style={{ ...s.container, backgroundColor: theme.bg, color: theme.text }}>
      ...
      <div style={s.mainWrapper}>
        ...
        {!isLoggedIn && !showLogin && (
          <div style={s.grid}>
             {/* أيقونات التواصل الاجتماعي كما هي */}
            ...
            <a href="https://instagram.com/shoqaq.ramallah/" target="_blank" rel="noreferrer" style={{ ...s.box, backgroundColor: theme.iconBox, border: `1.5px solid ${theme.border}` }}><Instagram size={24} color="#e1306c" /></a>
            <a href="https://tiktok.com/@shoqaq.ramallah" target="_blank" rel="noreferrer" style={{ ...s.box, backgroundColor: theme.iconBox, border: `1.5px solid ${theme.border}` }}>
        ...
        {showLogin && !isLoggedIn && (
          <div style={{ ...s.loginBox, backgroundColor: theme.cardBg, border: `1.5px solid ${theme.border}`, boxShadow: theme.shadow }}>
            ...
            <input type="tel" inputMode="numeric" placeholder="كلمة المرور" value={password} onChange={(e) => setPassword(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleLogin()} style={{ ...s.input, backgroundColor: theme.iconBox, border: `1px solid ${theme.border}`, color: theme.text, WebkitTextSecurity: 'disc' }} autoFocus />
          ...
          <div style={{ ...s.admin, border: `2px solid ${theme.border}`, backgroundColor: theme.cardBg, boxShadow: theme.shadow }}>
            <div style={s.adminHeader}>
              <div style={{ textAlign: 'right' }}>
                ...
                {/* لمبة فحص الاتصال بـ Supabase */}
                <div style={{ fontSize: '10px', color: dbStatus === 'connected' ? '#25D366' : '#ef4444', display: 'flex', alignItems: 'center', gap: '4px', marginTop: '4px' }}>
                  <CheckCircle2 size={12} />
                  {dbStatus === 'connected' ? 'Supabase متصل أونلاين' : 'فشل الاتصال بالقاعدة'}
                </div>
              </div>
...
const s = {
  ...
  serviceCard: { display: 'flex', alignItems: 'center', gap: '20px', padding: '22px 24px', borderRadius: '22px', border: '2px solid', transition: '0.2s ease' },
