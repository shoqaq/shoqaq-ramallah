...
function App() {
  ...
  // الألوان بناءً على الوضع المختار
  const theme = {
    bg: isDarkMode ? '#000000' : '#ffffff',
    text: isDarkMode ? '#ffffff' : '#1a1a1a',
    card: isDarkMode ? 'rgba(255,255,255,0.05)' : '#f9f9f9',
    border: isDarkMode ? 'rgba(255,255,255,0.1)' : '#eeeeee'
  };

  return (
    <div style={{ ...containerBase, backgroundColor: theme.bg, color: theme.text }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: '40px' }}>
        ...
        <h1 style={{ fontSize: '3rem', fontWeight: '900', margin: '15px 0 0 0' }}>
      ...
      {showLogin && !isLoggedIn && (
        <div style={{ marginBottom: '30px' }}>
          <input
            ...
            style={{ ...loginInput, borderColor: '#f59e0b', color: '#f59e0b' }}
            autoFocus
          />
        </div>
      )}
      ...
      {isLoggedIn ? (
        /* لوحة التحكم مع خيار تغيير الثيم */
        <div style={{ ...adminPanel, backgroundColor: theme.card, borderColor: theme.border }}>
          ...
          <div style={settingRow}>
            <span>وضع الصفحة (فاتح/غامق)</span>
            <button 
              onClick={() => setIsDarkMode(!isDarkMode)} 
              style={{ ...toggleBtn, backgroundColor: isDarkMode ? '#f59e0b' : '#333' }}
            >
              {isDarkMode ? 'الوضع الأسود' : 'الوضع الأبيض'}
            </button>
          </div>
        ...
        /* أزرار التواصل بأيقونات أصلية وملونة */
        <div style={socialGrid}>
          
          {/* واتساب */}
          <a href={`https://wa.me/${whatsappNumber}`} target="_blank" rel="noreferrer" style={{...iconBox, backgroundColor: '#25D366'}}>
          ...
          {/* اتصال */}
          <a href="tel:+970594560056" style={{...iconBox, backgroundColor: '#34A853'}}>
          ...
          {/* فيسبوك */}
          <a href="https://facebook.com/shoqaq.store/" target="_blank" rel="noreferrer" style={{...iconBox, backgroundColor: '#1877F2'}}>
          ...
          {/* تيك توك */}
          <a href="https://tiktok.com/@shoqaq.ramallah" target="_blank" rel="noreferrer" style={{...iconBox, backgroundColor: '#000000', border: '1px solid #fff'}}>
...
const socialGrid = { display: 'flex', gap: '15px', flexWrap: 'wrap', justifyContent: 'center' };
...
const footerText = { position: 'fixed', bottom: '20px', opacity: 0.3, fontSize: '11px', letterSpacing: '1px', fontWeight: 'bold' };
