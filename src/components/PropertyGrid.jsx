...
const PropertyGrid = ({ listings, onBack, onSelect, selectedProp, onCloseModal, theme }) => (
  <div style={s.fullWidth}>
    <div style={s.sectionHeader}>
      <button onClick={onBack} style={s.backBtn}><X size={18} /> رجوع</button>
      <h3 style={{margin:0}}>العقارات</h3>
    </div>
    
    <div style={s.gridDisplay}>
      {listings.map(item => (
        <div key={item.id} onClick={() => onSelect(item)} style={{ ...s.miniCard, backgroundColor: theme.cardBg, border: `1px solid ${theme.border}`, opacity: item.status === 'غير متاح' ? 0.5 : 1 }}>
      ...
      <div style={s.modalOverlay}>
        <div style={{ ...s.modalContent, backgroundColor: theme.cardBg, color: theme.text, border: `1px solid ${theme.border}` }}>
          <div style={s.modalHeader}>
            ...
            <X onClick={onCloseModal} style={{cursor:'pointer'}} />
          </div>
          <div style={s.modalBody}>
            ...
            <div style={s.modalLinks}>
               {selectedProp.post_url && <a href={selectedProp.post_url} target="_blank" style={s.actionLink}><ExternalLink size={16} /> رابط التفاصيل</a>}
